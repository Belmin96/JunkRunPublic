...
export default async function HaulerJobDetail({ params }: { params: Promise<{ id: string }> }) {
  ...
  return <div className="space-y-6">
    ...
    {job.status === 'ASSIGNED' && job.arrivalType === 'SET_TIME' && <PickupArrival jobId={job.id} scheduledAt={job.scheduledAt.toISOString()} pickupLatitude={job.pickupLatitude} pickupLongitude={job.pickupLongitude} arrivalVerifiedAt={job.arrivalVerifiedAt?.toISOString() ?? null} />}
