...
export async function POST(req: NextRequest) {
  ...
  await db.auditLog.create({ data: { actorUserId: user.id, action: 'JOB_CREATED', entityType: 'JOB', entityId: job.id, jobId: job.id, metadata: JSON.stringify({ timezone: job.timezone, scheduledAt: job.scheduledAt.toISOString() }) } })
