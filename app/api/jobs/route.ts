...
export async function GET(req: NextRequest) {
  ...
  if (user.role === 'HAULER') { const profile = await db.haulerProfile.findUnique({ where: { userId: user.id } }); if (!profile) return NextResponse.json([]); if (tab === 'mine') return NextResponse.json(await db.job.findMany({ where: { haulerId: profile.id }, orderBy: { updatedAt: 'desc' } })); const declines = await db.jobDecline.findMany({ where: { haulerId: profile.id }, select: { jobId: true } }); return NextResponse.json(await db.job.findMany({ where: { status: { in: ['POSTED', 'BIDDING'] }, id: { notIn: declines.map(d => d.jobId) } }, orderBy: { createdAt: 'desc' }, take: 100, include: { estimates: { where: { haulerId: profile.id } } } })) }
...
export async function POST(req: NextRequest) {
  ...
  const job = await db.job.create({ data: { jobNumber: newJobNumber(), customerId: user.id, jobTypes: JSON.stringify(data.jobTypes), whatToExpect: data.whatToExpect || null, numStories: data.numStories, pickupAddress: data.pickupAddress, city: data.city, zipCode: data.zipCode, arrivalType: data.arrivalType, date: data.date, time: data.arrivalType === 'SET_TIME' ? data.time! : null, scheduledAt, beforePhotoUrl: data.beforePhotoUrl, status: 'POSTED', paymentStatus: 'PENDING' } })
