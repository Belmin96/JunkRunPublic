...
export async function POST(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  if (user.role !== 'CUSTOMER' || estimate.job.customerId !== user.id) return NextResponse.json({ error: 'Only the customer who posted the job can approve an estimate' }, { status: 403 })
  ...
  const now = new Date()
  const pickupDeadlineAt = new Date(now.getTime() + PICKUP_DEADLINE_MINUTES * 60 * 1000)

  // Atomically claim the job before doing payment work. Only one estimate can win.
  const locked = await db.job.updateMany({
    where: { id, customerId: user.id, status: 'POSTED' },
  ...
  try {
    ...
    console.error('Estimate acceptance transaction failed', err)
