...
export async function POST(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  if (typeof reason !== 'string' || !CUSTOMER_REASONS.includes(reason as Reason)) {
    return NextResponse.json({ error: 'A valid cancellation reason is required', allowedReasons: CUSTOMER_REASONS }, { status: 422 })
  }
  ...
  if (['IN_PROGRESS', 'EVIDENCE_SUBMITTED', 'PENDING_PAYOUT'].includes(job.status)) {
    return NextResponse.json({ error: 'This job cannot be cancelled after work has started. Use the dispute process instead.' }, { status: 409 })
  }
  ...
  const updated = await db.$transaction(async (tx) => {
    const claimed = await tx.job.updateMany({
      ...
      data: { status: 'CANCELLED', cancellationReason: reason, cancelledAt, cancelledByUserId: user.id },
    })
    if (!claimed.count) return null
    await tx.auditLog.create({ data: { actorUserId: user.id, action: 'JOB_CANCELLED', entityType: 'JOB', entityId: id, jobId: id, metadata: JSON.stringify({ reason, cancelledAt: cancelledAt.toISOString() }) } })
