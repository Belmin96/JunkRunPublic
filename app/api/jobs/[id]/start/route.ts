...
export async function POST(_req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const now = new Date()
  if (job.pickupDeadlineAt && now >= job.pickupDeadlineAt) return NextResponse.json({ error: 'The 45-minute pickup window has expired. This job is being returned to the HaulBoard.' }, { status: 409 })
